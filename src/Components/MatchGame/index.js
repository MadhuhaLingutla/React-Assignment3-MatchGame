...
const initialstate = {
  ...
  score: 0,
  questionImage: imagesList[0].imageUrl,
...
class MatchGame extends Component {
  ...
  displayrespectivethumbnails = () => {
    const {selectedtab} = this.state

    const thumbnailsImagesList = imagesList.filter(
      each => each.category === selectedtab,
    )

    return (
      <ul className="thumbnails-container">
        {thumbnailsImagesList.map(each => (
          <li>
  ...
  selectfruits = () => {
    this.setState({selectedtab: tabsList[0].tabId})
  }

  selectanimals = () => {
    this.setState({selectedtab: tabsList[1].tabId})
  }

  selectplaces = () => {
    this.setState({selectedtab: tabsList[2].tabId})
  }

  compareimages = event => {
    const clickedthumbnailimage = event.target.src
    const {questionImage, score, elapsedseconds} = this.state
    const random = Math.floor(Math.random() * 30)

    const resultimage = imagesList.find(each => each.imageUrl === questionImage)

    if (clickedthumbnailimage === resultimage.thumbnailUrl) {
      if (elapsedseconds <= 60) {
        this.setState({
          score: score + 1,
          questionImage: imagesList[random].imageUrl,
        })
      } else {
        this.setState({displayscorecard: true})
      }
  ...
  displayoriginalcontent = () => {
    const {questionImage, selectedtab} = this.state

    const fruitsclassname =
      selectedtab === tabsList[0].tabId
        ? 'thumbnail-heading selectedtabclass'
        : 'thumbnail-heading'

    const animalsclassname =
      selectedtab === tabsList[1].tabId
        ? 'thumbnail-heading selectedtabclass'
        : 'thumbnail-heading'

    const placesclassname =
      selectedtab === tabsList[2].tabId
        ? 'thumbnail-heading selectedtabclass'
        : 'thumbnail-heading'

    return (
      <div className="question-thumbnails-wrapping-container">
        <div className="question-image-container">
          <img src={questionImage} alt="match" className="question-image" />
        </div>
        <div className="thumbnails-content-container">
          <ul className="thumbnails-heading-container">
            <li>
              <button
                className={fruitsclassname}
                onClick={this.selectfruits}
                type="button"
              >
          ...
          {this.displayrespectivethumbnails()}
        </div>
      </div>
    )
  }
  ...
  render() {
    const {elapsedseconds, displayscorecard, score} = this.state
          ...
          <div className="score-timer-container">
            ...
            <div className="timer-container">
              <img
                ...
                alt="timer"
              />
              <p className="remainingtime">{remainingtime} sec</p>
            </div>
          </div>
        </div>
